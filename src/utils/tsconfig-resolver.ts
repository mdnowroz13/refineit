...
function tryExtensions(prefixAbs: string) {
    ...
    candidates.push(normalizePath(prefixAbs)); // no extension
...
function packageExportsCandidates(pkgName: string) {
    try {
...
function monorepoPackageCandidates(pkgName: string) {
    const repoRoot = process.cwd();
    const pkFolder = path.join(repoRoot, "packages");
    const out: string[] = [];
    try {
        if (!fs.existsSync(pkFolder)) return out;
...
export function getCandidatesForImport(currentFile: string, importPath: string): string[] {
    ...
    const pushPrefix = (absPrefix: string) => {
    ...
    for (const m of mono) pushPrefix(m);

    const pkgMatches = packageExportsCandidates(importPath);
    for (const p of pkgMatches) pushPrefix(p);
...
export function resolveImportWithTsConfig(
    currentFile: string,
    importPath: string,
