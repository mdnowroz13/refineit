...
function parseArgs(): CLIArgs {
    ...
    for (let i = 0; i < argv.length; i++) {
        const a = argv[i];
        if (a === '--apply' || a === 'apply') out.apply = true;
        else if (a === '--yes' || a === '-y') out.yes = true;
        else if (a === '--dry-run') out.dryRun = true;
        else if (a === '--ci') out.ci = true;
        else if (a === '--export-report' && argv[i + 1]) { out.exportReport = argv[++i]; }
        else if (a === '--format' && argv[i + 1]) out.format = argv[++i] as any;
...
async function main() {
    ...
    table.push(
        ['🔴 Unused Imports', data.unusedImports.length],
        ['💀 Dead Files', data.deadFiles.length],
        ['👯 Duplicates', data.duplicatesCount],
        ['🔄 Circular Deps', data.cycles.length],
        ['📦 Large Files', data.largeFiles],
        ['📝 TODO Items', data.totalTodos],
        ['🔒 Security Risks', data.totalSecurity]
    );
    console.log('\n' + table.toString());
    ...
    while (true) {
        ...
        if (action === 'exit') { outro('Happy Coding!'); break; }
        ...
        if (action === 'fix') {
            ...
            for (const f of editFiles) { try { await backupFile(f, rootDir, 'modified'); } catch (e) { console.error('backup failed', f, e); } }
            for (const f of data.deadFiles) { try { await fs.unlink(f); } catch (e) { } }
            if (data.unusedImports.length > 0) await fixImports(data.unusedImports);
