...
function repoWideTextualConfirm(allFiles, identifier, excludePath) {
    for (const [p, content] of allFiles.entries()) {
        if (excludePath && normalizePath(p) === normalizePath(excludePath))
            continue;
        if (textualUsageCheck(content, identifier))
            return true;
    }
    return false;
}
...
export async function analyzeCodebase(files, whitelist = []) {
    let totalTodos = 0;
    let totalSecurity = 0;
    let largeFiles = 0;
    ...
    for (const [n, content] of fileContentsMap.entries()) {
        try {
            project.createSourceFile(n, content, { overwrite: true });
        }
        catch { }
    }
    for (const f of files) {
        ...
        const addImp = (mod) => {
            ...
            if (resolved)
                ...
                unresolvedImportsDebug.push({ importer: nf, importSpecifier: mod, candidates: getCandidatesForImport(nf, mod) });
        };
    ...
    for (const f of Array.from(fileLookup.values())) {
        ...
        if (!isImported && !isEntry && !isWhitelisted)
            deadFiles.push(f);
    }
